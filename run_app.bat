...
:: 1. Cleaned up Node.js path checking to prevent parenthesis crashes
...
powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort 3000 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess | Stop-Process -Force -ErrorAction SilentlyContinue" >nul 2>nul

:: 4. Launch through run_app.py (if Python is present) or directly via Node
where python >nul 2>nul
if !errorlevel! neq 0 goto launch_node

echo [LAUNCH] Starting application via launcher script...
python run_app.py
if !errorlevel! neq 0 (
    echo.
    echo [ERROR] python run_app.py exited with error code !errorlevel!
    pause
)
exit /b 0
